...
import com.example.projectfinal.menutype
import com.example.projectfinal.recycle_view
import com.facebook.*
...
class authen : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val login: Button = view.findViewById(R.id.log_in)

        login.setOnClickListener {
            val usrInput : EditText = view.findViewById(R.id.Textusername);
            val passInput : EditText = view.findViewById(R.id.TextPassword);
            ...
            if(msgUsrInput.trim().length>0&&passUsrInput.trim().length>0){
                ...
                transaction.replace(R.id.layout, menutype,"fragment_menutype")
                transaction.addToBackStack("fragment_menutype")
                transaction.commit()
    ...
    private fun handleFacebookAccessToken(token : AccessToken) {
        ...
        firebaseAuth!!.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                val fm = fragmentManager
                val transaction : FragmentTransaction = fm!!.beginTransaction()
                transaction.replace(R.id.layout, menutype,"menutype")
                transaction.addToBackStack("menutype")
                transaction.commit()
